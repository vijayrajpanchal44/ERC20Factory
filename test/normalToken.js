...
describe("Simple ERC20 token test", async function () {
    let ERC20Token, tokenContract, owner, addr1, addr2;

    beforeEach(async () => {
        ERC20Token = await ethers.getContractFactory("Token");

        tokenContract = await ERC20Token.deploy("My Token", "MTK", 18);
    ...
    it('can set decimals during construction', async function () {
        ...
        const tokenContract = await ERC20Token.deploy("My Token", "MTK", decimals);
